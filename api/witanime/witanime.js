...
export async function decryptWitanimeLinks(url) {
    let htmlContent;
    const proxyUrl = `http://217.154.201.164:7763/api/proxy?url=${encodeURIComponent(url)}`;
    ...
    for (let i = 0; i < totalLinks; i++) {
        ...
        const chunks = varsData[pVarName];  
         ...
         // تحديد الجودة بناءً على محتوى الرابط
         let quality = 'غير محدد';
         if (finalLink.includes('SD')) {
             quality = 'SD';
         } else if (finalLink.includes('HD')) {
             quality = 'HD';
         } else if (finalLink.includes('FHD')) {
             quality = 'FHD';
         } else {
             // محاولة استنتاج الجودة من فهرس الرابط (بافتراض الترتيب)
             const groupSize = Math.floor(totalLinks / 3); // 3 مجموعات (SD, HD, FHD)
             if (groupSize > 0) {
                 const groupIndex = Math.floor(i / groupSize);
                 if (groupIndex === 0) quality = 'SD';
                 else if (groupIndex === 1) quality = 'HD';
                 else if (groupIndex === 2) quality = 'FHD';
             }
         }
         
         // إضافة الرابط إلى القائمة  
         const name = serverNames[i] || `رابط ${i + 1}`;  
         finalLinks.push({ quality: quality, server: name, link: finalLink });
    }
